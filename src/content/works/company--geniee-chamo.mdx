...
- プロダクトチームと密に連携し、ユーザーフィードバックをもとに改善サイクルを設計・推進。ノードベースの可視化ワークフローシステムをゼロから設計・構築し、従来開発チームが手作業で行っていた設定フローを置き換えることで、営業担当者が数時間から数十分で顧客設定を完了できるようにし、導入リードタイムを大幅に短縮した
